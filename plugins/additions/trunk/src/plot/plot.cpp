...
// Modified by:
// Created:     12/01/2000
// RCS-ID:      $Id: plot.cpp,v 1.12 2005/08/10 22:53:35 MW Exp $
...
// XPMs
// ----------------------------------------------------------------------------
#include <wx/plot/res/plot_enl.xpm>
#include <wx/plot/res/plot_shr.xpm>
#include <wx/plot/res/plot_zin.xpm>
#include <wx/plot/res/plot_zot.xpm>
#include <wx/plot/res/plot_up.xpm>
#include <wx/plot/res/plot_dwn.xpm>
        ...
        : wxWindow( parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxSIMPLE_BORDER, _T("plotarea") )
{
	...
	this->SetMinSize( wxSize( 180, 140 ) );
}
...
{
    wxClientDC dc(this);
    wxPen aPen( *wxWHITE_PEN );
    m_owner->PrepareDC( dc );
    int wn, ws ;

    wn = curve->GetPenNormal().GetWidth();
    ws = curve->GetPenSelected().GetWidth();
    if (ws>wn)
        aPen.SetWidth(ws);
    else
        aPen.SetWidth(wn);
...
*/
        ...
        while (node)
        {
            ...
            if (curve == m_owner->GetCurrentCurve())
			{
                ...
                /*if (curve->GetPenSelected().GetWidth() < curve->GetPenNormal().GetWidth())
                {
                    aPen.SetWidth(curve->GetPenNormal().GetWidth()) ;
                    dc.SetPen(aPen) ;
            ...
            {
                /*if (curve->GetPenNormal().GetWidth() < curve->GetPenSelected().GetWidth())
                {
                    aPen.SetWidth(curve->GetPenSelected().GetWidth()) ;
                    dc.SetPen(aPen) ;
                    DrawCurve( &dc, curve, update_x-1, update_x+update_width+2 );
                }*/
                // changed by Stephane
...
#define  ID_ENLARGE       32767
#define  ID_SHRINK        32766

#define  ID_MOVE_UP       32765
#define  ID_MOVE_DOWN     32764

#define  ID_ZOOM_IN       32763
#define  ID_ZOOM_OUT      32762
          ...
          m_titleStaticText( NULL )
{
    ...
    m_plotAndTitleSizer->Add( plotsizer, 1, wxEXPAND | wxTOP | wxBOTTOM , 15 );
    ...
    if ((GetWindowStyleFlag() & wxPLOT_X_AXIS) != 0)
    {
        ...
        vert2->Add( m_area, 1, wxEXPAND|wxTOP| wxRIGHT,10 );
        vert2->Add( m_xaxis, 0, wxEXPAND| wxRIGHT, 10 );
    }
    else
    {
        plotsizer->Add( m_area, 1, wxEXPAND | wxRIGHT, 10);
...
void wxPlotWindow::DeleteAll()
{
	for( size_t i = 0; i < GetCount(); ++i )
	{
		m_curves.DeleteObject( GetAt( i ) );
	}
	m_area->Update();
}
